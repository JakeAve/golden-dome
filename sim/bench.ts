...
const a = parseArgs(Deno.args, { string: ['loadout', 'seeds', 'map'], default: { games: 20, map: 'valley', loadout: 'pac,pac,flak', cash: CFG.startCash, upgrade: 0 } });
...
for (let seed = s0; seed <= s1; seed++) {
  const g = createGame({ seed, cfg: { startCash: Number(a.cash) }, map: MAPS[String(a.map)] }) as any;
  ...
  for (const w of [10, 20, 30]) {
    g.run({ untilWave: w });
    cashAt[w] = g.phase === 'over' ? NaN : g.cash;
